"""Extract plays from FbPro98 gameplan (.pln) files.

Loads the underlying `PnflGamePlan` lazily and exposes the 64 normal plays and
10 custom special-teams plays as plain string lists, with optional sorting.

The reader holds a `PnflGamePlan` so any consumer that has a `GamePlanReader`
can also call `pnfl_gameplan.validate()` against the bound rule set. The
reader itself never validates and does not require the play pool to be
populated — `play_pool` defaults to an empty `PlayPool` when not supplied.
"""

from __future__ import annotations

from os import PathLike
from pathlib import Path
from typing import Literal, TextIO

from pnfl_gameplan import PNFL_RULES, PnflGamePlan, PnflRules
from pnfl_playpool import PlayPool

StrPath = str | PathLike[str]
SortOrder = Literal["slot", "name"]

NORMAL_HEADER = "=== Normal ==="
SPECIAL_HEADER = "=== Special ==="


class GamePlanReader:
    """Reads a .pln file and exposes its plays as string lists, lazy-loading on first access.

    Wraps a `PnflGamePlan` internally. `rules` and `play_pool` default to the
    canonical PNFL rule set and an empty pool — extracting play names does not
    require pool resolution. Callers that need validation pass an inhabited
    pool and call `pnfl_gameplan.validate()` on the loaded reader.
    """

    def __init__(
        self,
        gameplan_path: StrPath,
        *,
        rules: PnflRules = PNFL_RULES,
        play_pool: PlayPool | None = None,
    ) -> None:
        self.gameplan_path = Path(gameplan_path)
        self._rules = rules
        self._play_pool = play_pool if play_pool is not None else PlayPool(".")
        self._pnfl_gameplan: PnflGamePlan | None = None

    @property
    def pnfl_gameplan(self) -> PnflGamePlan:
        """The bound `PnflGamePlan` (loaded on first access)."""
        if self._pnfl_gameplan is None:
            self._pnfl_gameplan = PnflGamePlan.from_file(str(self.gameplan_path), self._rules, self._play_pool)
        return self._pnfl_gameplan

    def get_normal_plays(self, sort: SortOrder = "slot") -> list[str]:
        """Return the 64 normal plays.

        With sort="slot" (default), preserves slot positions — empty slots are
        returned as empty strings, so the output is always 64 entries.
        With sort="name", filters out empty slots and sorts case-insensitively
        by play name.
        """
        if sort == "slot":
            return self._get_normal_plays_by_slot()
        return sorted(
            (name for name in self._get_normal_plays_by_slot() if name),
            key=str.casefold,
        )

    def get_custom_special_plays(self) -> list[str]:
        gameplan = self.pnfl_gameplan.gameplan
        return ["" if play is None else play.name for play in gameplan.custom_special_plays]

    def write_normal_plays(self, stream: TextIO, *, sort: SortOrder = "slot") -> None:
        stream.write("\n".join(self.get_normal_plays(sort)) + "\n")

    def write_custom_special_plays(self, stream: TextIO) -> None:
        stream.write("\n".join(self.get_custom_special_plays()) + "\n")

    def _get_normal_plays_by_slot(self) -> list[str]:
        gameplan = self.pnfl_gameplan.gameplan
        return ["" if play is None else play.name for play in gameplan.normal_plays]
